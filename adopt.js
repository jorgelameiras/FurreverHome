...
function updatePetGrid(pets) {
    ...
    pets.forEach((pet) => {
        ...
        const isFavorited = (pet.adopterId === curUserId);
...
async function toggleFavorite(petId) {
    try {
        const response = await fetch('http://localhost:5500/addFavorite', {
            method: 'POST',
...
async function getCurrentUser() {
    try {
        const response = await fetch('http://localhost:5500/currentUser', {
            method: 'GET',
            headers: { 'Content-Type': 'application/json' },
        });
